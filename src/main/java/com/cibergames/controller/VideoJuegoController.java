...
public class VideoJuegoController {
	...
	public String grabarJuegos(@ModelAttribute VideoJuego juego, Model model, 
		...
		try {
			...
			if(!imagen.isEmpty()) { //Si la imagen no es vacia
				...
				juego.setRuta_img_juego("default.jpg");
			}
			}
			catch(Exception ex) {
				ex.printStackTrace();
			...
			model.addAttribute("mensajeConfirmacion", "Juego Registrado correctamente");
			
		} 
		catch (Exception e) {
			e.printStackTrace();
			model.addAttribute("mensajeFallido", "Algo ha ocurrido. No ha podido registrarse el juego");
		}
	...
	public String eliminarJuego(@ModelAttribute VideoJuego videojuego, Model model) {
		...
		model.addAttribute("eliminado", "Eliminacion realizada");
		}
		catch (Exception e) {
		model.addAttribute("fallido", "Fallo la eliminacion");
		}
		//Lista de videojuegos
		model.addAttribute("listaVideoJuegos", repoVideoJuego.findAll());
